...
public interface AuditoriaRepositorioJpa extends JpaRepository<AuditoriaEntidad, UUID> {
    ...
    @Query("""
        ...
        where (:accion is null or lower(auditoria.accion) = lower(:accion))
          and (:tipoEntidad is null or lower(auditoria.tipoEntidad) = lower(:tipoEntidad))
          and (:usuarioActor is null or lower(usuario.usuario) = lower(:usuarioActor))
